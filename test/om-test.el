...
(ert-deftest om--diary-sexp/valid-props ()
  (om--compare-element-props
   (om-build-diary-sexp '(value)) "%%(value)"))
...
(ert-deftest om--match-filter/index ()
  (let ((contents '("one" "two" "three")))
    (should (equal (om--match-filter 0 contents) '("one")))
    (should (equal (om--match-filter '(> 0) contents) '("two" "three")))
    (should (equal (om--match-filter '(>= 1) contents) '("two" "three")))
    (should (equal (om--match-filter '(< 2) contents) '("one" "two")))
    (should (equal (om--match-filter '(<= 1) contents) '("one" "two")))
    (should (equal (om--match-filter -1 contents) '("three")))
    (should-not (om--match-filter 3 contents))
    (should-not (om--match-filter -4 contents))))
