...
(ert-deftest om-elem--match-filter/index ()
  (let ((contents '("one" "two" "three")))
    (should (equal (om-elem--match-filter 0 contents) '("one")))
    (should (equal (om-elem--match-filter '(> 0) contents) '("two" "three")))
    (should (equal (om-elem--match-filter '(>= 1) contents) '("two" "three")))
    (should (equal (om-elem--match-filter '(< 2) contents) '("one" "two")))
    (should (equal (om-elem--match-filter '(<= 1) contents) '("one" "two")))
    (should (equal (om-elem--match-filter -1 contents) '("three")))
    (should-not (om-elem--match-filter 3 contents))
    (should-not (om-elem--match-filter -4 contents))))

(ert-deftest om-elem--match-filter/type ()
  (let ((contents (list (om-elem-build-bold "text1")
                        (om-elem-build-bold "text2")
                        (om-elem-build-italic "text3"))))
    (should (equal (->> (om-elem--match-filter 'bold contents)
                        (--map (om-elem-to-trimmed-string it)))
                   '("*text1*" "*text2*")))
    (should (equal (->> (om-elem--match-filter 'italic contents)
                        (--map (om-elem-to-trimmed-string it)))
                   '("/text3/")))
    (should-not (->> (om-elem--match-filter 'underline contents)
                     (--map (om-elem-to-trimmed-string it))))))

(ert-deftest om-elem--match-filter/property ()
  (let ((contents
    ...
    (should (equal (->> (om-elem--match-filter '(:checkbox off) contents)
                        (--map (om-elem-to-trimmed-string it)))
                   '("- [ ] one ::")))
    (should (equal (->> (om-elem--match-filter '(:checkbox on) contents)
                        (--map (om-elem-to-trimmed-string it)))
                   '("- [X] two ::" "- [X] three ::")))
    (should-not (->> (om-elem--match-filter '(:checkbox trans) contents)
                     (--map (om-elem-to-trimmed-string it))))))

(ert-deftest om-elem--match-filter/compound ()
  (let ((contents
         (list
    ...
    (should (equal (->> (om-elem--match-filter
    ...
    (should (equal (->> (om-elem--match-filter
                        ...
                        (--map (om-elem-to-trimmed-string it)))
                   '("* DONE headline2")))
    (should-not (->> (om-elem--match-filter
                      '(:and headline (:todo-keyword "CANC")) contents)
                     (--map (om-elem-to-trimmed-string it))))))

(ert-deftest om-elem--match-filter/error ()
  (should-error (om-elem--match-filter "no-strings" t))
  (should-error (om-elem--match-filter :no-keywords t))
  ;; these should not be by themselves
  (should-error (om-elem--match-filter '(<) t))
  (should-error (om-elem--match-filter '(<=) t))
  (should-error (om-elem--match-filter '(>) t))
  (should-error (om-elem--match-filter '(>=) t))
  (should-error (om-elem--match-filter '(:and) t))
  (should-error (om-elem--match-filter '(:or) t))
  ;; numeric operators must be followed by integer
  (should-error (om-elem--match-filter '(< 1.0) t))
  (should-error (om-elem--match-filter '(< 'no-symbols) t))
  (should-error (om-elem--match-filter '(< :no-keywords) t))
  (should-error (om-elem--match-filter '(< "no-string") t))
  ;; only one index allowed
  (should-error (om-elem--match-filter '(< 1 2) t)))
