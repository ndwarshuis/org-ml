...
;; TODO somehow make this work on windows?
(defun org-ml-bench-get-sys-info ()
  (let ((cpumodel (->> (shell-command-to-string "lscpu | grep 'Model name:'")
                       (s-chop-prefix "Model name:")
                       (s-trim)))
        (memtotal (->> (shell-command-to-string "grep MemTotal /proc/meminfo")
